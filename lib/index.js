...
class Router {
   ...
   * @property {string}  [prefix='']          string to prefix when the router is used by another
  ...
  async handle(ctx, next = () => {}) {
    ...
    // done a wrapper for next
    // which is called to go on for next route/router before doing some cleaning job
    async function done() {
      ...
      // call next to go on for next route/router
      return next();
    }

    async function routerNext(signal) {
      // remove added slash
      if (slashAdded) {
      ...
      while (match !== true && idx < stack.length) {
        layer = stack[idx];
        idx += 1;
        match = matchLayer(layer, path, ctx);
        route = layer.route;
      ...
      return self.process_params(layer, paramcalled, ctx, async (paramSignal) => {
        if (paramSignal) {
          return routerNext(paramSignal);
        }
        if (route) {
          // ==> route.handle(ctx, next)
          return layer.handle_request(ctx, routerNext);
        }
  ...
  async process_params(layer, called, ctx, next) {
    ...
    async function paramCallback(signal) {
      const paramMid = paramCallbacks[paramIndex];
      paramIndex += 1;
      ...
      return paramMid(ctx, paramCallback, paramVal, key.name);
    }
  ...
  get(path, ...middlewares) {
    return this.method('get', path, ...middlewares);
  }
  ...
  post(path, ...middlewares) {
    return this.method('post', path, ...middlewares);
  }
  ...
  delete(path, ...middlewares) {
    return this.method('delete', path, ...middlewares);
  }
  ...
  put(path, ...middlewares) {
    return this.method('put', path, ...middlewares);
  }
  ...
  patch(path, ...middlewares) {
    return this.method('patch', path, ...middlewares);
  }
  ...
  head(path, ...middlewares) {
    return this.method('head', path, ...middlewares);
  }
  ...
  options(path, ...middlewares) {
    return this.method('options', path, ...middlewares);
  }
...
function matchLayer(layer, path, ctx) {
  return layer.match(path, ctx);
}
