...
class Layer {
  ...
  /**
   * check whether actualQuery matches current layer
  ...
  queryMatch(actualQuery) {
    ...
    return this.queryKeys.every((key) => {
      const check = this.queryCheckers[key];
      return check(this._query[key], actualQuery[key]);
    });
  }
}
